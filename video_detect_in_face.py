        ...
        if len(str_lp)<9:  
            ...
            if flag:
                ...
                if len(above) == 4 and len(below) >=4 and len(below) <=5 and below.isnumeric() == True and above[0:2].isnumeric() == True and above[2].isnumeric() == False: 
                    ...
                    # print((time.time() - detectTime ),end = "\t")

                    cv2.putText(frame,str_lp,(50, 50), cv2.FONT_HERSHEY_PLAIN, 3, (0, 255, 255),2, lineType=cv2.LINE_AA)
                    cv2.namedWindow('Reult')
                    cv2.setWindowProperty('Reult',cv2.WND_PROP_FULLSCREEN,cv2.WINDOW_FULLSCREEN)
        ...
        if finish_detect == 1:
            ...
            #Them moi nguoi dung
                ...
                if sampleNum <5:
                    ...
                    if reset == 1 and time.time()-start_time > 0:
                        # print((time.time() - detectTime ))
                        now = datetime.now()
                        fm_time = now.strftime("%H:%M:%S %d/%m/%Y")
                        status = 'IN'
                        ...
                        ok = 1
                        # print("----------------------NEXT-----------------------")
        cv2.namedWindow('checkin-face')
        cv2.setWindowProperty('checkin-face',cv2.WND_PROP_FULLSCREEN,cv2.WINDOW_FULLSCREEN)
        cv2.imshow('checkin-face',frame)
        # if have_lp == 1:
            # time.sleep(10)
            # have_lp = 0
        if out > 1510:
