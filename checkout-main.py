    ...
    while(cap.isOpened()):
        ...
        if len(str_lp)<9:  
            ...
            if flag:
                ...
                if len(above) == 4 and len(below) >=4 and len(below) <=5 and below.isnumeric() == True and above[0:2].isnumeric() == True and above[2].isnumeric() == False: 
                    ...
                    # print(list)
                    # if len(list)==10:
                    #     str_lp = find_max_str(list)
                    #     xacsuat = list.count(str_lp)/len(list)
                    
                    # print("Xac xuat: ",xacsuat)

                    #kiem tra bien so co dang ki hay chua
                    cvt_str_lp = str_lp.replace(str_lp,"'"+str_lp+"'")
                    cur = conn.cursor()
                    query = "SELECT id, studentID, newUser FROM time_IO where LincensePlate="+cvt_str_lp+"ORDER BY id DESC LIMIT 1"
                    cur.execute(query)
                    result = cur.fetchall()
                    if result:
                        ...
                        if check_new_user == 0:
                            ...
                            query = "SELECT studentID FROM lp_Registered where LincensePlate="+cvt_str_lp
        ...
        if finish_detect == 1:
            if next == 1:
                ...
                cv2.putText(frame,succes,(50, 50), cv2.FONT_HERSHEY_PLAIN, 3, (0, 255, 0),3, lineType=cv2.LINE_AA)
                if time.time()-start_time > 5:
                    ...
                    print("----------------------NEXT-----------------------")
            if next == 0:
                cv2.putText(frame,fail,(50, 30), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 255),2, lineType=cv2.LINE_AA)
                ...
                if adminAccept == 1:
                    ...
                    if time.time()- wait_5s >5:
                        ...
                        if model == 2:
                            ...
                            cvt_str_lp = str_lp.replace(str_lp,"'"+str_lp+"'")
                            id_cur = conn.cursor()
                            query = "SELECT studentID FROM time_IO where LincensePlate="+cvt_str_lp+"ORDER BY id DESC LIMIT 1"
                        ...
                        fm_time = now.strftime("%H:%M:%S %d/%m/%Y")
                        status = 'OUT'
                        ins = "INSERT INTO time_IO(studentID,LincensePlate,time,status,newUser) VALUES('AdminAccept','"+str_lp+"','"+fm_time+"','"+status+"',0)"
                        print("Insert succesfully!!")
                        print(ins)
                        # conn.execute(ins)
                        # conn.commit()
